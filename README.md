...
> This will create open a link to the current directory with a random dynamic port and run for 15 minutes.
...
> This takes the path parameter followed by the port and finally the runtime in minute, minutes can also be taken as floating point.

- *To open the link from another machine on your local area network just [find your device's ip](https://www.wikihow.com/Find-Out-Your-IP-Address) address and replace `localhost` with your ip say `192.168.43.136` to `http://192.168.43.136:8080`.*

- *You can visit the link and you'll such an interface.*

![web interface](/resources/web-interface.png)

- *To download press the download section.*

- *If feel like closing the connection before the elapse tim press `ctrl+c` which will clean up release system resources and close the connection.*
...
### To share over the network you need to do something known as port forwarding or tunneling
...
- Run the script normally from one terminal and open a separate one and run.
...
> NOTE: The link is not consistent and you may be assigned a new link if you restart your cloudflared connection.

- *`ctrl+c` to close the cloudflared connection, **for advanced setup you can go through [cloudflared documentation](https://developers.cloudflare.com/fundamentals/account/create-account/) and create to get your domain**.*

- *For more info on the tool run `cloudflared --help` on your commandline or from their github repo.*
